craft_bases = {
    "detect": {
        ...
        "others":
        "if entity @e[type=minecraft:item,distance=..1,"
        "nbt={Item:{id:\"%%ID%%\",Count:%%COUNT%%b}}] ",
        "block":
        "if block %%BLOCK_POS%% %%BLOCK_ID%% ",
        "tag":
        "run tag @s add %%PROJECT_NAME_LCF%%WillCraft%%CRAFT_NAME_UCF%%\n"
    },
    "clear_others":
    "execute as @e[type=minecraft:item,tag=%%PROJECT_NAME_LCF%%WillCraft%%CRAFT_NAME_UCF%%] at @s "
